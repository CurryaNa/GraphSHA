...
def train():
    ...
    with torch.no_grad():
        ...
        val_loss = focal_loss(output[data_val_mask], data.y[data_val_mask])
    optimizer.step()
...
log_dir = os.path.join('run','train5')
train_writer = SummaryWriter(log_dir=log_dir)

log_dir = os.path.join('run','test5')
...
for epoch in tqdm.tqdm(range(args.epoch)):
    ...
    test_writer.add_scalar('Test Accuracy', tmp_test_acc, epoch)
